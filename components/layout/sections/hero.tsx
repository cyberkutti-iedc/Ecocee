...
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ArrowRight, Lightbulb, X, Terminal, Sparkles } from "lucide-react";
import { useRouter } from "next/navigation";
import { useState, useEffect } from "react";

export const HeroSection = () => {
  const router = useRouter();
  const [showKuttAIBanner, setShowKuttAIBanner] = useState(true);
  ...
  return (
    <section className="relative w-full max-w-full overflow-hidden">
      {/* KuttAI Beta Announcement Banner */}
      {showKuttAIBanner && (
        <div className="fixed top-4 right-4 z-50 animate-fade-in">
          <div className="relative bg-gradient-to-r from-green-500 to-blue-600 text-white p-4 rounded-lg shadow-xl max-w-sm border border-green-300/30">
            <button 
              onClick={() => setShowKuttAIBanner(false)}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm mt-1 text-white/90">
                  Experience our new AI assistant for embedded development. Join the waitlist now!
                </p>
                
                <Button 
                  onClick={handleKuttAIClick}
                  className="mt-3 bg-white text-green-700 hover:bg-gray-100 font-bold text-xs py-1 h-8"
                >
                  Join Beta
                </Button>
              </div>
            ...
            <div className="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-green-400 to-blue-500 rounded-full"></div>
          </div>
        </div>
      )}
        ...
        <div className="relative flex flex-col lg:flex-row items-center justify-between min-h-[85vh] sm:min-h-[75vh] py-8 sm:py-12 lg:py-24 gap-8 lg:gap-12">
          ...
          <div className="flex-1 w-full flex flex-col items-center lg:items-start text-center lg:text-left space-y-6 sm:space-y-8 order-2 lg:order-1">
            ...
            <div className="mt-2">
              <Badge 
                variant="outline" 
                className="bg-gradient-to-r from-green-500 to-blue-600 text-white border-0 cursor-pointer hover:from-green-600 hover:to-blue-700 transition-all py-2 px-4 text-xs font-bold animate-pulse"
                onClick={handleKuttAIClick}
              >
                <Terminal className="w-3 h-3 mr-1" />
                Introducing KuttAI Beta - Join Waitlist
              </Badge>
            </div>
          ...
          <div className="flex-1 w-full flex items-center justify-center max-w-lg mx-auto lg:mx-0 order-1 lg:order-2">
            <div className="relative w-64 h-64 sm:w-80 sm:h-80 lg:w-96 lg:h-96 max-w-full">
              ...
              >
                ...
                {/* KuttAI floating badge on logo */}
                <div 
                  className="absolute top-4 right-4 bg-gradient-to-r from-green-500 to-blue-600 text-white px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 shadow-lg cursor-pointer hover:scale-105 transition-transform"
                  onClick={handleKuttAIClick}
                >
                  <Terminal className="w-3 h-3" />
                  <span>KuttAI Beta</span>
                </div>
