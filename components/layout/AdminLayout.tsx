...
import ProductsAdminDashboard from "../ProductsAdminDashboard";
...
type AdminTab = "roles" | "careers" | "modules" | "Calendar" | "Games" | "certificates" | "revenue" | "bookings" | "products";
...
const handleThemeChange = (theme: typeof colorThemes[0]) => {
  ...
  async function handleCertDelete(id: number) {
    ...
    try {
      await fetch(`/api/internship-certificate?id=${id}`, { method: "DELETE" });
      fetchCertificates();
    } catch (e: any) {
  ...
  async function handleCertFormSubmit(e: React.FormEvent) {
    ...
    try {
      if (editCert) {
        // Update
        await fetch(`/api/internship-certificate?id=${editCert.id}`, {
          method: "PUT",
        ...
        // Insert
        await fetch("/api/internship-certificate", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(formCert),
        });
  ...
  const tabItems = [
    ...
    },
    {
      ...
      count: 0,
      description: "Manage internship certificate records"
    },
    {
      id: "revenue" as const,
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab === "products" && <ProductsAdminDashboard/>}
            </motion.div>
          </AnimatePresence>
        </div>
      </main>
